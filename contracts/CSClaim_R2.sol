...
contract CSClaim_R2 is AccessControlUpgradeable, PausableUpgradeable, UUPSUpgradeable {
  ...
  uint public feePercentage; // Xe18 Platform fee
  ...
  struct PoolInfo {
    address owner; // Owner will receive fee amount of the pool
    IERC20 token;
    bytes32 root;
  ...
  function initialize(address _owner, address _receiver, uint _feePercentage) public initializer {
    ...
    require(_feePercentage <= FULL_100, 'Invalid fee');
  ...
  function claim(uint256 id, uint256 index, uint256 _totalAmount, bytes32[] memory _proof) external whenNotPaused {
    ...
    if (_maxClaim > claimed) {
      ...
      if (pool.feePercentage > 0) {
        ...
        pool.token.safeTransfer(pool.owner, _feeAmount); // Transfer pool fee to pool owner
      }
      if (feePercentage > 0) {
        uint256 _platformFeeAmount;
        unchecked {
          _platformFeeAmount = (_claimableAmount * feePercentage) / FULL_100;
          _totalFeeAmount += _platformFeeAmount;
        }
        pool.token.safeTransfer(receiver, _platformFeeAmount); // Transfer platform fee to contract receiver
      }

      _claimableAmount -= _totalFeeAmount;
      emit Claimed(msg.sender, _claimableAmount, _totalFeeAmount);
      pool.token.safeTransfer(msg.sender, _claimableAmount); // Transfer token
    }
  }
  ...
  function setSchedules(uint256 id, ScheduleVesting[] calldata _schedules) external onlyAdminOrOwner(id) {
    delete schedules[id];
    ScheduleVesting[] storage poolSchedules = schedules[id];
    ...
    for (uint256 i = 0; i < _schedules.length; ) {
      ...
      poolSchedules.push(schedule);
      lastPercent = schedule.unlockPercent;
      unchecked {
        i++;
      }
    }
    require(lastPercent <= FULL_100, 'Invalid percent');
  }
